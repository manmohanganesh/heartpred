...
def evaluate_models(X_test,y_test,model_path='models'):
    model_files = [f for f in os.listdir(model_path) if f.endswith('_model.pk1')]
    results = {}
    models_dict = {}

    for model_file in model_files:
        model_name = model_file.replace('_model.pk1','').capitalize()
        print(f"\n Loading {model_name}..")
        ...
        print(f"📊 Accuracy: {acc:.4f}, Precision: {prec:.4f}, Recall: {rec:.4f}, AUC: {auc_score:.4f}")#, F1: {f1:.4f}")
        ...
        input_example = X_test[:5]  # first 5 rows as example
